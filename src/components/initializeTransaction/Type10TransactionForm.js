...
export default class Type10TransactionForm extends React.Component {
    ...
    async signTransaction() {
        try {
            ...
            var alias = { senderPublicKey: senderPublicKey, sender: sender, alias: this.state.alias };

            signer.setProvider(new ProviderWeb(config.provider));

            const signedAlias = await signer.alias(alias).sign();
            this.setState({ signedTransaction: signedAlias, showSignedTransaction: true });
        } catch(err) { }
    };
                ...
                <div className="col-xl-12 col-xxl-12">
                    <div className="card">
                        ...
                        <div className="card-body">
                            ...
                            >
                                <label className="text-label">Alias</label>
                                <input
                                    type="text"
                                    name="alias"
                                    className="form-control"
                                    onChange={ (event) => { this.aliasChanged(event); } }
                                    required
                                />
                                <br />
